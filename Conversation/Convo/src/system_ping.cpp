...
int system_ping::ping_ip_address(std::string ip_address) //, int max_attempts, std::string& details)
{
    ...
    try
    {
        command = "ping -c 1 " + ip_address + " 2>&1";
...
int system_ping::test_connection (std::string ip_address, bool check_eth_port) //, int max_attempts, bool check_eth_port, int eth_port_number)
{
    ...
    try
    {
        if (check_eth_port)
        {
            std::ifstream eth_conn_status ("/sys/class/net/eth0/carrier");

            eth_conn_status >> eth_conn_status_int; // 0: not connected; 1: connected
            if (eth_conn_status_int != 1)
            {
                //std::cerr << __FILE__ << "(" << __FUNCTION__ << ":" << __LINE__ << ") | eth0 unplugged";
                return -1;
            }
        }

        if (ping_ip_address(ip_address) != 1) //, max_attempts, details) != 1)
        {
